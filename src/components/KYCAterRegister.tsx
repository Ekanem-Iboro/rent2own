...
const KYCAfterReg = () => {
  ...
  const uploaded = localStorage.getItem("uploaded") || null;
  ...
  return (
    <>
      ...
      >
        {/* Show upload loading if file exists and upload is in progress */}
        {uploaded === "false" && isUploading > 0 && isUploading != 100 && (
          <UploadLoading uploadProgress={isUploading} />
        )}

        {/* Show success if file is uploaded */}
        {uploaded === "true" && (
          <UploadSuccess handleClearUpload={handleClearUpload} file={file!} />
        )}

        {/* Show file upload component when no file is uploaded or after clearing upload */}
        {!file && uploaded === "false" && (
          <UploadFile
...
function UploadSuccess({ handleClearUpload, file }: successUpload) {
  return (
